<?php
  ...
  if (empty($_POST[mail])) {
    header("Location: ../create.php?err=Please enter your email.\n");
    exit();
}
  if ( empty($_POST[passwd])) {
    header("Location: ../create.php?err=Password can't be blank.\n");
    exit();
}
  if ( empty($_POST[passwd2])) {
    header("Location: ../create.php?err=Please confirm the password.\n");
    exit();
}
  ...
  if ($sth->fetchColumn()) {
      header("Location: ../create.php?err=Acount already taken.\n");
      exit();
  }
  ...
  $headers = 'From:smtetwa@student'."\r\n";
  mail($to, $subject, $message, $headers);
  header("Location: ../index.php?err=Thank you for creating an account. The link has been sent to your email.\n");
